...
export default function EventCreateForm({ event = null, isEdit = false, eventCategories = [] }) {
    const initialEvent     = event?.data ?? event ?? {};
    const existingImageUrl = initialEvent.media?.url ?? initialEvent.image?.url ?? null;
    ...
    return (
        <form onSubmit={handleSubmit} className="max-w-4xl p-6 mx-auto space-y-10 rounded-xl">
            ...
            <section className="space-y-1">
                ...
                <div className="border-t border-brand-border pt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <select value={data.event_category_id}
                            onChange={(e) => setData("event_category_id", e.target.value)}
                            className={`p-2 border rounded border-brand-border focus:ring-2 focus:ring-brand-secondary outline-none bg-white ${errors.event_category_id ? "border-red-500" : ""}`}>
                            <option value="">Seleciona uma categoria</option>
                            {eventCategories.map((cat) => {
                                const title = getLocalizedValue(cat.title);
                                return (
                        ...
                        {errors.event_category_id && <span className="text-red-500 text-sm">{errors.event_category_id}</span>}
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <input type="text" value={data.location}
                            ...
                            className={`p-2 border rounded border-brand-border focus:ring-2 focus:ring-brand-secondary outline-none ${errors.location ? "border-red-500" : ""}`} />
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <input type="date" value={data.start_date}
                            ...
                            className={`p-2 border rounded border-brand-border focus:ring-2 focus:ring-brand-secondary outline-none ${errors.start_date ? "border-red-500" : ""}`} />
                    ...
                    <div className="flex flex-col gap-2">
                        <div className="flex items-center justify-between">
                            ...
                            {data.end_date && (
                                <button type="button" onClick={() => setData("end_date", "")}
                        ...
                        <input type="date" value={data.end_date}
                            ...
                            className={`p-2 border rounded border-brand-border focus:ring-2 focus:ring-brand-secondary outline-none ${errors.end_date ? "border-red-500" : ""}`} />
            ...
            <section className="space-y-1">
                ...
                <div className="border-t border-brand-border pt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="flex flex-col gap-2">
                        <label className="font-semibold text-brand-black">Descrição (PT)</label>
                        <textarea rows={6} value={data.description.pt}
                            onChange={(e) => setData("description", { ...data.description, pt: e.target.value })}
                            className={`p-2 border rounded border-brand-border focus:ring-2 focus:ring-brand-secondary outline-none resize-y ${errors["description.pt"] ? "border-red-500" : ""}`} />
                        {errors["description.pt"] && <span className="text-red-500 text-sm">{errors["description.pt"]}</span>}
                    </div>
                    <div className="flex flex-col gap-2">
                        <label className="font-semibold text-brand-black">Descrição (EN)</label>
                        <textarea rows={6} value={data.description.en}
                            onChange={(e) => setData("description", { ...data.description, en: e.target.value })}
                            className={`p-2 border rounded border-brand-border focus:ring-2 focus:ring-brand-secondary outline-none resize-y ${errors["description.en"] ? "border-red-500" : ""}`} />
                        {errors["description.en"] && <span className="text-red-500 text-sm">{errors["description.en"]}</span>}
                    </div>
                </div>
            </section>
            ...
            <section className="space-y-1">
                ...
                <div className="border-t border-brand-border pt-4">
                    <ImageUpload
                        ...
                        onChange={(file, has) => setData("image", file)}
                        error={errors.image}
                    />
                </div>
            </section>
