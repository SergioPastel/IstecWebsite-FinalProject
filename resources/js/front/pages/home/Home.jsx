        ...
        <section className="w-full mt-[120px]">
          ...
          >
            {bannerImages.length > 0 ? (
              ...
              // Fallback slide — shown when no banner images are configured
              <SwiperSlide>
          ...
          <div className="max-w-[1600px] mx-auto px-6">
            ...
            <div className="grid grid-cols-3 gap-[22px] max-[1100px]:grid-cols-2 max-[768px]:grid-cols-1">
              {courses.data.map((course) => (
                <div
                  key={course.id}
                  className={`min-h-full bg-white border border-[#dbe4ee] rounded-[20px] p-6 shadow-[0_8px_24px_rgba(15,23,42,0.06)] hover:-translate-y-1 transition-all duration-300 flex flex-col ${getCourseCardHoverClasses(course.category)}`}
                >
                  <span
                    className={`inline-flex self-start items-center font-extrabold ${getCourseBadgeClasses(course.category)}`}
                  >
                    {course.category?.startsWith("CTeSP")
                      ? "CTeSP"
                      : course.category}
                  </span>
          ...
          <div className="max-w-[1600px] mx-auto px-6">
            ...
            <div className="grid grid-cols-3 gap-[22px] max-[1100px]:grid-cols-2 max-[768px]:grid-cols-1">
              {latestActivity.data.map((item) => (
                <div
                  key={item.id}
                  className={`min-h-full bg-white border border-[#dbe4ee] rounded-[20px] p-6 shadow-[0_8px_24px_rgba(15,23,42,0.06)] hover:-translate-y-1 transition-all duration-300 flex flex-col ${getEventCardHoverClasses(item.type)}`}
                >
                  <span
                    className={`inline-flex self-start items-center min-h-[30px] px-3 py-[6px] rounded-full text-[0.8rem] font-extrabold ${getEventBadgeClasses(item.type)}`}
                  >
                    {item.type}
                  </span>
                  <h3 className="mt-[14px] mb-[10px] text-[1.2rem] leading-[1.3] font-semibold">
          ...
          <div className="max-w-[1600px] mx-auto px-6">
            ...
            <div className="flex justify-center">
              <button
                ...
                onClick={() =>
                  router.visit(route("applications.courses.apply"))
                }
              >
